...
if func == 0x01 then
if _G.u2_addr == dev or dev == 0xFA then
...
modbus_resp(_G.u2_addr, func, string.format("%02X%s", nbytes, hex_payload))
end
...
modbus_resp(_G.u2_addr, func, string.format("%02X%s", bytlens, strhex))
else
modbus_resp(_G.u2_addr, func + 0x80, string.format("%02x", 0x02))
end
end
...
modbus_resp(_G.u2_addr, func, strhex)
elseif func == 0x06 then
local strhex = string.format("%04X%04X", bytstart, bytlen)
_G.handle_modbus_write(bytstart, bytlen, false)
modbus_resp(_G.u2_addr, func, strhex)
else
modbus_resp(_G.u2_addr, func + 0x80, string.format("%02x", 0x01))
end
end
...
log.info("u2.10", reg, val, dlen, tmpdat:toHex())
local status, err = pcall(_G.handle_modbus_write, reg, tmpdat, true)
if not status then log.error("u2.e", err) end
local strhex = ""
